...
def run_ddqn_example(env_class, env_name, num_episodes=10000):
    ...
    agent = DoubleDQNAgent(env, state_size=state_size, action_size=action_size, gamma=gamma, lr=learning_rate,  target_update_freq=target_update_freq)
    ...
    plt.title(f"DDQN Learning Curve - {env_name}")
    plt.xlabel("Episode")
    plt.ylabel("Score")
    plt.legend()

    plt.savefig(f"src/metrics/plot/d_dqn/ddqn_{env_name.lower().replace(' ', '_')}_learning_curve.png")
    plt.close()
...
def main():
    for env_class, env_name in [
        ...
        scores = run_ddqn_example(env_class, env_name)
