...
from src.agents.reinforce_baseline_learned_by_critic import REINFORCEBaselineLearnedByCriticAgent
...
def create_tensorboard_writer(env_name: str) -> SummaryWriter:
    ...
    log_dir = os.path.join('runs', 'reinforce_baseline_learned_by_critic', env_name, current_time)
...
def run_reinforce_baseline_learned_by_critic_example(
    env_class,
    env_name,
    num_episodes=100000,
    lr_policy=0.001,
    ...
    agent = REINFORCEBaselineLearnedByCriticAgent(
        env=env,
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % 10000 == 0:
            base_dir = os.path.join(os.getcwd(), 'model', 'reinforce_baseline_learned_by_critic', env_name)
    ...
    final_path = os.path.join(os.getcwd(), 'model', 'reinforce_baseline_learned_by_critic',
...
def main():
    os.makedirs("src/metrics/plot/reinforce_baseline_learned_by_critic", exist_ok=True)
    ...
    for (env_class, env_name), config in env_configs.items():
        print(f"\nRunning REINFORCE with baseline learned by a critic on {env_name}...")  # Updated message
        scores, steps, times = run_reinforce_baseline_learned_by_critic_example(
            env_class,
            env_name,
            **config
        )
        ...
        plt.tight_layout()
        plt.savefig(
            f"src/metrics/plot/reinforce_baseline_learned_by_critic/reinforce_baseline_learned_by_critic_{env_name.lower().replace(' ', '_')}_metrics.png"
        )
        plt.close()
